...
def random_sleep(dummy_argument=None) -> float:
    sleep_duration = random.uniform(0, 10)  # Generate a random sleep duration between 0 and 10 seconds
...
def call_random_sleep_parallel() -> None:
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        results = executor.map(random_sleep, [None] * 20)  # Pass a list of 20 None values as the argument
