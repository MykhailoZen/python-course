...
if __name__ == '__main__':
    ...
    print("Sequential Result:", calculate_sum(1, 2 ** 30))
    duration_sequential = time.time() - start_time
    print(f"Sequential Duration: {duration_sequential:.2f} seconds")

    start_time = time.time()
    print("Parallel Result:", calculate_sum_parallel(1, 2 ** 30, 10))
    duration_parallel = time.time() - start_time
    print(f"Parallel Duration: {duration_parallel:.2f} seconds")
