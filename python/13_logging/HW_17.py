...
which returns the sum of numbers in the given 
range including both ends. Create another 
function (e.g. "calculate_sum_parallel") that calls "calculate_sum" in a 
parallel manner 
...
The returning result should be 
the same as for the first function. 
...
def calculate_sum_parallel(start: int, end: int, num_processes: int):
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        futures = [
            executor.submit(calculate_sum, i, min(i + chunk_size, end + 1))
